...
class ViewTask extends React.Component {
	...
	handleData(response) {
		this.requestApplicant(response.wfRequest.applicantWorkId).then((res) => {
			...
			this.setState({
				...
				isReplace,
				assocList: response.wfRequest.items,
	...
	render() {
		const assetColumns = [
			...
			{
				dataKey: 'requestStartDate',
				title: '借用时间',
				width: 200,
				hidden: this.state.headerInfo.title ? this.state.headerInfo.title.indexOf('借用') === -1 : true,
				render: (title, rowData) => moment(title).format('YYYY-MM-DD') + ' ~ ' + moment(rowData.requestEndDate).format('YYYY-MM-DD'),
			},
			{
				dataKey: 'creator',
				title: '责任人',
